...
def sinus():
    ...
    Grand_X1 = np.fft.fftshift(np.fft.fft(x1))
    Grand_X1 = 20 * np.log10(Grand_X1)
    freq_x1 = np.fft.fftfreq(2048, d=1 / fe)
    positive_freq_x1 = freq_x1[freq_x1 >= 0]
    Grand_X1_positive = Grand_X1[freq_x1 >= 0]

    Grand_X2 = np.fft.fftshift(np.fft.fft(x2))
    Grand_X2 = 20 * np.log10(Grand_X2)
    freq_x2 = np.fft.fftfreq(500, d=1 / fe)
    positive_freq_x2 = freq_x2[freq_x2 >= 0]
    Grand_X2_positive = Grand_X2[freq_x2 >= 0]
    ...
    axs1[1].set_title('Spectre du sinus a 200Hz')
