...
static int32_t inBuffer1[SIG_LEN], inBuffer2[SIG_LEN], outBuffer1[SIG_LEN], outBuffer2[SIG_LEN],
        debugBuffer1[FFT_LEN], debugBuffer2[FFT_LEN],debugBuffer3[FFT_LEN],debugBuffer4[FFT_LEN],debugBuffer5[FFT_LEN], Fe;
        ...
        if (inputBufferFull) {
            ...
            if (SWT_GetValue_Local(0) == 0) { // MAIN DECISION TREE: PASS-THROUGH
                ...
                for(n=0;n < FFT_LEN; n++)
                {
                    ...
                    inFFT[n].im = -(outFFT[n].im * Htot[n].re + outFFT[n].re * Htot[n].im);
                }
